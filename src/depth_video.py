...
class DepthVideo:
    def __init__(self, cfg, args):
        ...
        self.counter = Value('i', 0) # 当前关键帧计数
        self.ready = Value('i', 0) # 是否准备好
        self.mapping = Value('i', 0) # 是否在进行mapping
        self.ba_lock = { #BA锁
            'dense': Value('i', 0), # dense BA
            'loop': Value('i', 0), # loop closing
        }
        self.global_ba_lock = Value('i', 0) # 全局BA锁
        ht = cfg['cam']['H_out'] # 480
        self.ht = ht
        wd = cfg['cam']['W_out'] # 640
        self.wd = wd
        self.stereo = (cfg['mode'] == 'stereo')
        device = args.device
        self.device = device
        c = 1 if not self.stereo else 2
        self.scale_factor = 8 # 缩放因子
        s = self.scale_factor
        buffer = cfg['tracking']['buffer'] #512

        ### state attributes ### 可以在线程间共享的状态属性
        self.timestamp = torch.zeros(buffer, device=device, dtype=torch.float).share_memory_() # 时间戳
        self.images = torch.zeros(buffer, 3, ht, wd, device=device, dtype=torch.float) # 图像
        self.dirty = torch.zeros(buffer, device=device, dtype=torch.bool).share_memory_()
        self.red = torch.zeros(buffer, device=device, dtype=torch.bool).share_memory_() # 是否需要重定位
        ...
        self.disps_up = torch.zeros(buffer, ht, wd, device=device, dtype=torch.float).share_memory_() # [b, h, w]
        self.intrinsics = torch.zeros(buffer, 4, device=device, dtype=torch.float).share_memory_() # fx, fy, cx, cy

        ### feature attributes ### 可以在线程间共享的特征属性
        ...
        ### initialize poses to identity transformation，初始化位姿为单位矩阵
    ...
    def get_lock(self):
        return self.counter.get_lock() # 获取锁

    def get_ba_lock(self, ba_type):
        return self.ba_lock[ba_type].get_lock() # 获取BA锁

    def get_mapping_lock(self):
        return self.mapping.get_lock() # 获取mapping锁

    # __item_setter这个函数的主要作用是更新关键帧计数器，并将传入的项目（如时间戳、图像、位姿、视差等）设置到相应的属性中。
    def __item_setter(self, index, item):
        if isinstance(index, int) and index >= self.counter.value:
            self.counter.value = index + 1
        elif isinstance(index, torch.Tensor) and index.max().item() > self.counter.value:
            self.counter.value = index.max().item() + 1 # 更新关键帧计数器的值。
    ...
    ###  dense mapping operations ### 稠密建图操作
    def get_mapping_item(self, index, device='cuda:0', decay=0.1):
        with self.mapping.get_lock():
            ...
            est_disp = self.disps_filtered[index].clone().to(device)  # [h, w]，估计的视差
            ...
            return image, depth, c2w, gt_c2w, mask # 返回图像、深度、位姿、真实位姿、mask
    ...
    @staticmethod
    def format_indices(ii, jj, device='cuda'): # 格式化索引
        """ to device, long, {-1}"""
    ...
    def reproject(self, ii, jj):
        """ project points from ii -> jj """  # 从ii投影到jj
        ii, jj = DepthVideo.format_indices(ii, jj, self.device)
        Gs = lietorch.SE3(self.poses[None, ...])
        #将点从帧ii投影到帧jj。这个函数需要输入位姿、深度、内参以及源帧和目标帧的索引。它会返回投影后的坐标和有效掩码。
    ...
    def distance(self, ii=None, jj=None, beta=0.3, bidirectional=True):
        ...
        # 帧的光流距离度量，其中距离= sqrt((u(ii) - u(jj->ii))^2 + (v(ii) - v(jj->ii))^2)
        return_matrix = False
        ...
        ii, jj = DepthVideo.format_indices(ii, jj) # 格式化索引

        intrinsic_common_id = 0  # we assume the intrinsic within one scene is the same，我们假设一个场景内的内参是相同的
        if bidirectional:
            poses = self.poses[:self.counter.value].clone() # 位姿
